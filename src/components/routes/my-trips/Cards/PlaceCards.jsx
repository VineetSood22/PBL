...
function PlaceCards({ place }) {
  ...
  const getPlaceInfo = async () => {
    // Clean the query string to avoid mismatched or invalid addresses
    const cleanedCity = city ? city.replace(/,?\s*Himachal Pradesh\s*\d{5,6}/i, "").trim() : "";
    const data = {
      textQuery: place.name + " " + cleanedCity,
    };
    ...
    try {
      ...
      if (!place) {
        console.warn("No place details found for query:", data.textQuery);
        return;
      }
      let info = {
        id: place.id,
