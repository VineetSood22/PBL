...
import { getCityDetails, PHOTO_URL, libraries } from "@/Service/GlobalApi";
...
function Locationinfo() {
  ...
  const getCityInfo = async () => {
    ...
    console.log("🏙️ getCityInfo called with city:", city, "type:", typeof city);

    // Validate city data
    if (!city) {
      console.warn("⚠️ No city data available");
      return;
    }

    // Ensure city is in the correct format for the API
    let queryCity = city;
    if (typeof city === "object" && city !== null) {
      // If city is an object, try to extract a meaningful string
      if (city.displayName) {
        queryCity = city.displayName;
      } else if (city.formattedAddress) {
        queryCity = city.formattedAddress;
      } else if (city.name) {
        queryCity = city.name;
      } else {
        console.warn("⚠️ City object doesn't have expected properties:", city);
        return;
      }
    }

    try {
      console.log("🏙️ Calling getCityDetails with:", queryCity);
      const result = await getCityDetails(queryCity);
      if (result && result.length > 0) {
        setCityDets(result[0]);
        setAllImages(result[0].photos || []);
        setPhotos(result[0].photos?.[0]?.name || "");
      } else {
        console.warn("⚠️ No city details found for:", queryCity);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (photos) {
      ...
      setUrl("/images/main_img_placeholder.jpg");
    }
  }, [photos]);
